...
export function resolveElement(element: React.ReactElement, isPrimitiveComponent: (type: any) => boolean): React.ReactElement | null {
	if (typeof element.type === 'function' && !isPrimitiveComponent(element.type)) {
		...
		if (element.type.prototype && element.type.prototype.isReactComponent) {
			...
			// Function component
			return (element.type as any)(element.props);
		}
	}
	return element;
}
...
export function renderElementToString(element: React.ReactElement): string {
	...
	if (typeof element.type === 'function') {
		...
		if (element.type.prototype && element.type.prototype.isReactComponent) {
			// Class component
			const ComponentClass = element.type as new (props: any) => React.Component;
			const instance = new ComponentClass(element.props);
			rendered = instance.render();
		} else {
			// Function component
			const FunctionComponent = element.type as (props: any) => React.ReactNode;
...
export function renderChildrenToString(children: React.ReactNode): string | undefined {
	if (typeof children === 'string') {
		console.log("String children raw:", JSON.stringify(children));
